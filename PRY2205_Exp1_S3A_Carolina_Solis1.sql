...
SELECT 
    ...
    CASE 
        WHEN c.celular_cli IS NOT NULL THEN                                              
            '0' || SUBSTR(TO_CHAR(c.celular_cli), 1, 1) || '-' ||      
            SUBSTR(TO_CHAR(c.celular_cli), 2, 3) || '-' || 
            SUBSTR(TO_CHAR(c.celular_cli), 5, 4)
        ELSE 'SIN CELULAR'
...
SELECT 
    ...
    TO_CHAR(ROUND(AVG(e.sueldo_emp)), 'FM$9G999G999') AS "SUELDO_PROMEDIO"
...
SELECT 
    ...
    TO_CHAR(ROUND(AVG(p.valor_arriendo)), 'FM$999G999G999') AS "PROMEDIO_ARRIENDO",
    TO_CHAR(AVG(p.superficie), 'FM999G999D00') AS "PROMEDIO_SUPERFICIE",
    TO_CHAR(ROUND(AVG(p.valor_arriendo / p.superficie)), 'FM$999G999') AS "VALOR_ARRIENDO_M2",
    CASE 
